...
def validate_lines_in_file(fileName, file_contents, evaluationParams, withTranscription=False, withConfidence=False,
    ...
    """
    This function validates that all lines of the file calling the Line parameter_validation function for each line
    """
...
def get_tl_line_values_from_file_contents(content, evaluation_params, withTranscription=False, withConfidence=False,
    ...
    for line in lines:
        line = line.replace("\r", "").replace("\n", "")
        if line != "":
            points, angle, confidence, transcription = get_tl_line_values(line, evaluation_params, withTranscription,
...
def main_evaluation(p, evalParams, validate_data_fn, evaluate_method_fn, show_result=True, per_sample=True):
    ...
    # gt = p['g']
    # subm = p['s']
    evalData, prec, rec, confs = evaluate_method_fn(gt, subm, evalParams)
    ...
    return resDict['method'], prec, rec, confs


def main_evaluation_notzip(p, evalParams, validate_data_fn, evaluate_method_fn, show_result=True, per_sample=True):
    """
    This process validates a method, evaluates it and if it succed generates a ZIP file with a JSON entry for each sample.
    Params:
    p: Dictionary of parmeters with the GT/submission locations. If None is passed, the parameters send by the system are used.
